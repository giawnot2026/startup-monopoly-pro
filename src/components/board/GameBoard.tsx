...
import { Trophy, Award, Skull, Home, ArrowRight } from 'lucide-react';
...
export default function GameBoard({ 
  ...
  // NUOVO STATO: Gestisce se il giocatore locale ha già mosso in questo turno
  ...
  useEffect(() => {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { 
          ...
          if (!isMyTurnNow || newState.currentPlayerIndex !== players.indexOf(currentPlayer)) {
            ...
            // Se il turno è passato a me, resetto la possibilità di muovere
            if (isMyTurnNow) setHasMoved(false);
          }
        })
        .subscribe();
    ...
    fetchAndSubscribe();
  }, [roomCode, localPlayerName, isSyncing, isRolling, players, currentPlayer, setCurrentPlayerIndex, setPlayers]); 
  ...
  // LOGICA CHIUSURA MODAL: Ora NON passa il turno, lo chiude e basta
  const handleCloseModal = useCallback(() => {
    setModalConfig({ isOpen: false });
    // Sincronizziamo lo stato degli asset/denaro ma restiamo sullo stesso giocatore
    syncGameState(players, players.indexOf(currentPlayer));
  }, [players, currentPlayer, syncGameState]);

  // FUNZIONE PER PASSARE IL TURNO MANUALMENTE
  const handleEndTurn = useCallback(() => {
    const nextIdx = (players.indexOf(currentPlayer) + 1) % players.length;
    let finalNextIdx = nextIdx;
    
    // Salta eventuali giocatori in bancarotta
    while (players[finalNextIdx].isBankrupt && finalNextIdx !== players.indexOf(currentPlayer)) {
      finalNextIdx = (finalNextIdx + 1) % players.length;
    }

    setHasMoved(false);
    nextTurn(); // Aggiorna stato locale
    syncGameState(players, finalNextIdx); // Aggiorna DB
  }, [players, currentPlayer, nextTurn, syncGameState]);

  const handleDiceRoll = () => {
    if (!currentPlayer || currentPlayer.name !== localPlayerName || hasMoved) return;
    ...
    const shuffleInterval = setInterval(() => {
      ...
      if (++counter >= 12) {
        ...
        setTimeout(() => {
          ...
          setIsRolling(false);
          setHasMoved(true); // SEGNA CHE IL GIOCATORE HA MOSSO
          
          setPlayers([...updatedPlayers]);
          
          // Sincronizza il movimento ma MANTIENE il turno corrente
        ...
        <div className="absolute inset-[25%] flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[3rem] z-20 p-6 text-center">
          ...
          {!hasMoved ? (
            ...
            <button 
              ...
              className="px-10 py-3 bg-emerald-600 hover:bg-emerald-500 font-black rounded-xl text-white text-[10px] font-mono transition-all flex items-center gap-2 shadow-lg shadow-emerald-600/20 animate-bounce"
            >
              <ArrowRight size={14} /> TERMINA TURNO
            </button>
          )}
        </div>
