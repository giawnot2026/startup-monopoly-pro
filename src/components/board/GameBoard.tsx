...
export default function GameBoard({ initialPlayers }: { initialPlayers: any[] }) {
  ...
  const processTile = (tile: any) => {
    ...
    if (tileName.includes("exit")) {
      ...
      setModalConfig({
        ...
        description: canExit ? "Congratulazioni! Hai i numeri per vendere la startup." : "Requisiti non soddisfatti (Valutation > 1M e Equity > 0).",
    ...
    // 2. GESTIONE FUNDING (ANGOLI E PITCH DECK)
    // Controllo potenziato: se il nome contiene "pitch", "funding", "investor" o "round"
    const isFundingTile = tile.type === 'special' && 
      (tileName.includes("pitch") || tileName.includes("funding") || tileName.includes("investor") || tileName.includes("round"));
    ...
    if (tile.type === 'asset' && tile.badges) {
      setModalConfig({ 
        ...
        type: 'success', 
        title: tile.name, 
        description: "Vuoi potenziare questo asset?", 
        actionLabel: "Investi", 
        onAction: () => { upgradeBadge(tile.id); setModalConfig({ isOpen: false }); nextTurn(); }, 
        onClose: () => { setModalConfig({ isOpen: false }); nextTurn(); } 
      });
    } 
    // 5. EVENTI SPECIALI (OPPORTUNITÀ/IMPREVISTI)
    else if (tile.type === 'special' && (tileName.includes("opportunità") || tileName.includes("imprevisti"))) {
      const isOpp = tileName.includes("opportunità");
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 p-4 max-w-[1600px] mx-auto min-h-screen items-start font-sans bg-slate-950">
      ...
      <div className="relative w-full lg:w-[800px] aspect-square bg-slate-900 p-4 border border-blue-500/20 rounded-[2.5rem] shadow-2xl">
        ...
        {/* HUD CENTRALE */}
        <div className="absolute inset-[25%] flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[3rem] z-20 p-6 shadow-2xl">
          ...
          <div className={`w-16 h-16 mb-4 flex items-center justify-center rounded-2xl border-2 transition-all duration-150 ${
            isRolling ? 'scale-110 border-blue-500 shadow-[0_0_20px_rgba(37,99,235,0.4)] rotate-12' : 'border-white/10 rotate-0'
          } bg-slate-800 text-white text-3xl font-black`}>
            {diceValue || '?'}
          </div>

          <div className="text-center mb-6">
            <div className="text-2xl font-black text-white tracking-tighter italic">€{valuation.toLocaleString()}</div>
            <span className="text-blue-400 font-mono text-[7px] uppercase tracking-widest opacity-60">Valuation attuale</span>
          </div>

          <button 
            onClick={handleDiceRoll} 
            disabled={isRolling || modalConfig.isOpen} 
            className="px-10 py-3 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-xl transition-all uppercase tracking-widest text-[10px]"
          >
            ...
            return (
              <div key={tile.id} style={{ gridRow: row, gridColumn: col }} className="relative h-full w-full">
                ...
                <div className="absolute bottom-1 left-1 flex gap-0.5 z-30">
                  {playersHere.map(p => <div key={p.id} className="w-2 h-2 rounded-full border border-white" style={{ backgroundColor: p.color }} />)}
                </div>
              </div>
            );
          })}
      ...
      <div className="w-full lg:w-[350px] space-y-3">
        <h3 className="text-blue-400 font-black tracking-widest uppercase text-[10px] mb-2 px-2">Market Overview</h3>
        {players.map((p) => {
          ...
          return (
            <div key={p.id} className={`p-4 rounded-2xl border transition-all duration-500 ${isTurn ? 'bg-blue-600/20 border-blue-500 shadow-lg' : 'bg-slate-900/50 border-white/5 opacity-80'} ${p.isBankrupt ? 'grayscale opacity-50' : ''}`}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[9px] font-mono text-blue-400">{p.equity.toFixed(0)}% EQ</span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                <div className="bg-black/20 p-2 rounded-lg">
                  <span className="text-slate-500 block text-[7px] uppercase font-bold">Cash</span>
                  <span className="text-white font-mono">€{p.cash.toLocaleString()}</span>
                </div>
                <div className="bg-black/20 p-2 rounded-lg">
                  <span className="text-slate-500 block text-[7px] uppercase font-bold">EBITDA</span>
                  <span className={`font-mono ${(p.mrr - p.monthlyCosts) >= 0 ? 'text-green-400' : 'text-red-400'}`}>€{(p.mrr - p.monthlyCosts).toLocaleString()}</span>
                </div>
              </div>
