...
interface ActionModalProps {
  ...
  title: string;
  description: string;
  insight?: string; // Messaggio educativo
  badgeCta?: string; // Call to action personalizzata per i Badge
  impact?: {
    cash?: number;
    details?: string;
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
      <div className={`${config.bg} ${config.border} border-2 w-full max-w-lg rounded-[2.5rem] p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden transition-all animate-in fade-in zoom-in duration-300`}>
        ...
        <div className="text-center mt-4">
          ...
          {/* Sezione Insight Educativo */}
          {insight && (
            <div className="mb-4 px-2">
              <p className="text-slate-300 text-[11px] leading-relaxed italic opacity-80 border-l-2 border-white/10 pl-3 text-left">
                {insight}
              </p>
            </div>
          )}

          <p className="text-slate-400 text-[10px] leading-tight mb-6 px-4">{description}</p>

          {/* Box Impatto Funzionale */}
          {impact && (
            <div className="bg-black/40 rounded-2xl p-4 mb-6 border border-white/5">
               <p className="text-[8px] uppercase text-slate-500 font-bold mb-1 tracking-widest text-center italic">Analisi Impatto Startup</p>
                ...
                {assetLevels.map((level) => (
                  <div 
                    ...
                    className={`relative p-3 rounded-2xl border-2 flex flex-col items-center transition-all duration-300 ${
                      level.status === 'owned' ? 'bg-emerald-500/10 border-emerald-500 opacity-100' :
                  ...
                  >
                    ...
                    <div className="text-[8px] font-mono font-bold text-blue-400 italic">
                      €{(Number(level.cost) || 0).toLocaleString()}
                    </div>
                    
                    {level.status === 'owned' && (
                      <div className="absolute -top-2 -right-2 bg-emerald-500 text-black rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-black">✓</div>
                    )}
                  </div>
                ))}
              </div>
